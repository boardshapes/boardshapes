...
func generateRegion(img image.Image) *regions.Region {
	region := make(regions.Region, 0, img.Bounds().Dx()*img.Bounds().Dy())
	for y := img.Bounds().Min.Y; y < img.Bounds().Max.Y; y++ {
		for x := img.Bounds().Min.X; x < img.Bounds().Max.X; x++ {
			if c := color.NRGBAModel.Convert(img.At(x, y)); c != colors.White {
				region = append(region, regions.Pixel{X: uint16(x), Y: uint16(y)})
			}
		}
	}
	return &region
...
func generateShape(img image.Image) *[]geometry.Vertex {
	shape := make([]geometry.Vertex, 0, img.Bounds().Dx()*img.Bounds().Dy())
	var x, y int

Y:
	for y = img.Bounds().Min.Y; y < img.Bounds().Max.Y; y++ {
		for x = img.Bounds().Min.X; x < img.Bounds().Max.X; x++ {
			if img.At(x, y) != colors.Blank {
				break Y
			}
		}
	}
	...
	for {
		shape = append(shape, geometry.Vertex{X: uint16(x), Y: uint16(y)})
...
func generateTestResultImage(name string, shape *[]geometry.Vertex) error {
	...
	for i, v := range *shape {
		...
		img.Set(int(v.X), int(v.Y), colors.Red)
		images[i] = &img
		delays[i] = 10
	}
...
func TestRegion_CreateShape(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotShapeP, err := CreateShapeFromRegion(tt.region)
			if err != nil {
