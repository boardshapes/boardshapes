...
func TestBuildRegionMap(t *testing.T) {
	...
	for _, tt := range regionTests {
		t.Run(tt.name, func(t *testing.T) {
			img := imageops.SimplifyImage(tt.args.img, imageops.ImageSimplificationOptions{
				AllowWhite: tt.args.options.AllowWhite,
			})
...
func BenchmarkBuildRegionMap(b *testing.B) {
	for _, bm := range regionTests {
		b.Run(bm.name, func(b *testing.B) {
			img := imageops.SimplifyImage(bm.args.img, imageops.ImageSimplificationOptions{
				AllowWhite: bm.args.options.AllowWhite,
			})
			for b.Loop() {
